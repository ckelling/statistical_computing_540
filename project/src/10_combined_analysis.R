...
time_df <- c(mean(sgd_store$df$time), mean(adam_store$df$time),
...
iter_df <- c(mean(sgd_store$df$iter), mean(adam_store$df$iter),
             mean(nadam_store$df$iter),mean(adam_store_nobc$df$iter),
             mean(nadam_store_nobc$df$iter))

citer_df <- c(mean(sgd_store$df$c_iter), mean(adam_store$df$c_iter),
             mean(nadam_store$df$c_iter),mean(adam_store_nobc$df$c_iter),
             mean(nadam_store_nobc$df$c_iter))

titer_df <- c(mean(sgd_store$df$tot_iter), mean(adam_store$df$tot_iter),
...
colnames(full_comp) <- c("sgd", "adam", "nadam", "adam_nobc", "nadam_nobc")
...
plot_dat <- rbind(sgd_df, adam_df, nadam_df, adam_df_nobc, nadam_df_nobc)
...
n <- 350
plot_dat2 <- rbind(sgd_df[c(1:n, half:(half+n-1)),], adam_df[c(1:n, half:(half+n-1)),], 
